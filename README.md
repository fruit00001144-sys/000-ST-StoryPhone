...
- 微信私聊：文字、图片、表情包文字贴纸、后台生成回复，事件写入 `phone_wechat`，用 `type=npc_chat` 区分。
- 群聊：群成员由 profile `groups` 配置，消息写入 `phone_wechat`，用 `type=group_message` 区分，只对群成员可见。
- Phone Profile：每张角色卡自动创建/切换轻量 Profile，保存 hostChar、NPC identities、contacts、groups、forumBoards、momentsConfig，并镜像到旧 UI 所需的 friends/groups。
...
- 给 `style.css` 也加版本参数，避免 SillyTavern / 浏览器继续缓存旧 UI 样式。

## 0.4.4 诊断版

- 改用 `style-0.4.4.css` 文件名，绕开部分环境对 CSS query 参数缓存/解析不稳定的问题。
- 手机桌面会显示 `v0.4.4`，用于确认当前真正加载的是最新版。
...
{
  "id": "string",
  "source": "main_chat | phone_wechat | phone_moments | phone_forum | phone_calendar | phone_memo | target_phone",
  "type": "string",
  "actor": "string",
  "target": "string|null",
  "content": "string",
...
STStoryPhoneKnowledge.getCurrentProfile()
STStoryPhoneKnowledge.listProfiles()
STStoryPhoneKnowledge.createProfileForCurrentCard()
STStoryPhoneKnowledge.exportProfile()
STStoryPhoneKnowledge.getGroupMembers('group_a')
